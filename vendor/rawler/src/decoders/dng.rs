  ...
  fn full_image(&self, file: &RawSource, params: &RawDecodeParams) -> Result<Option<DynamicImage>> {
    ...
    if let Some(sub_ifds) = self.tiff.root_ifd().get_sub_ifd_all(TiffCommonTag::SubIFDs) {
      let first_ifd = sub_ifds
        .iter()
        .filter(|ifd| ifd.get_entry(TiffCommonTag::NewSubFileType).map(|entry| entry.force_u32(0)) == Some(1))
        .max_by_key(|ifd| {
          let width = ifd.get_entry(TiffCommonTag::ImageWidth).map(|entry| entry.force_u32(0)).unwrap_or(0);
          let height = ifd.get_entry(TiffCommonTag::ImageLength).map(|entry| entry.force_u32(0)).unwrap_or(0);
