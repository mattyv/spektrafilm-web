...
impl Default for RawDevelop {
  fn default() -> Self {
    Self {
      ...
      demosaic_method: DemosaicMethod::Ppg,
    }
  }
}
        ...
        RawPhotometricInterpretation::Cfa(config) => {
          if let Intermediate::Monochrome(pixels) = intermediate {
            ...
            if config.cfa.is_rgb() {
              Intermediate::ThreeColor(match self.demosaic_method {
                DemosaicMethod::Ppg => PPGDemosaic::new().demosaic(&pixels, &config.cfa, &config.colors, roi),
                DemosaicMethod::Superpixel => Superpixel3Channel::new().demosaic(&pixels, &config.cfa, &config.colors, roi),
              })
            } else if config.cfa.unique_colors() == 4 {
              Intermediate::FourColor(match self.demosaic_method {
                DemosaicMethod::Ppg => Bilinear4Channel::new().demosaic(&pixels, &config.cfa, &config.colors, roi),
                DemosaicMethod::Superpixel => Superpixel4Channel::new().demosaic(&pixels, &config.cfa, &config.colors, roi),
              })
            } else {
              todo!()
            }
